...
import {
	getRynoDividerHeaderFilter,
	getRynoDividerHueRotation,
} from "../../lib";
...
export const Header = ({ src, ...props }: BoxProps<"img">) => {
	...
	const svg = getRynoDividerHeaderFilter(divider);
	const hue = getRynoDividerHueRotation(divider);

	const filter = isUndefined(svg)
		? isUndefined(hue)
			? "grayscale(1)"
			: `hue-rotate(${hue}deg)`
		: `url(#header${divider.id})`;
	...
	const img = <Image {...props} src={src} sx={sx} />;
	return isUndefined(svg) ? (
		img
	) : (
		<>
			{svg}
